...
1. **Откройте Telegram и найдите целевой бот** `@smeshnoto4kabot`
2. **Отправьте ему команду `/start`** (это нужно сделать от вашего личного аккаунта)
3. **Используйте команду `/get_chat_id`** в вашем боте для получения chat_id
4. **Добавьте chat_id в переменные окружения** на Railway

### Важно!

После получения chat_id добавьте его в переменные окружения:
- В локальном `.env` файле: `TARGET_BOT_CHAT_ID=полученный_chat_id`
- На Railway: добавьте переменную `TARGET_BOT_CHAT_ID` со значением полученного chat_id
